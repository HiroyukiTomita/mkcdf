c mk_ofuro_nc_ltmm_cera20c_v1.1.f 
c
c This is a sample program to create netCDF file 
c for CERA20C Monthly 1DEG grid variable for a year
c
c PARAMETERS
c  VVAARR: Variable Name (QA, LHF...)
c  UUNNIITT: unit (g/kg, w/m^2)
c  LLOONNGG: Long name (J-OFURO3 QA V1.0)
c  IINNPPUUTT: Input file (a binary file must have "YYYY1-YYYY2.bin")
c  OOUUTTPPUUTT: Input file (a netCDF file must have "YYYY1-YYYY2.nc")
c  YYYY1: Start year
c  YYYY2: End year 
c  UUNNIITT: unit (g/kg, w/m^2)
c-----------------------------------------------------------------------  
...
C     $Id: pres_temp_4D_wr.f,v 1.11 2007/01/24 19:45:09 russ Exp $
      ...
      program mk_ofuro_nc_monthly_v1
      implicit none
      include 'netcdf.inc'
...
C     timesteps of data.
      integer NDIMS, NRECS
      parameter (NDIMS = 3, NRECS = (YYYY2-YYYY1+1)*12 ) 
      integer NLATS, NLONS
      parameter (NLATS = 181, NLONS = 360)
...
C     We recommend that each variable carry a "units" attribute.
      ...
      parameter (REC_UNITS = 'days since YYYY1-01-01')
...
c     For monthly data
      integer jd, jdmon(12)
      data jdmon/31,28,31,30,31,30,31,31,30,31,30,31/

C     Create pretend data. If this wasnt an example program, we would
C     have some real data to write, for example, model output.
      do lat = 1, NLATS
         lats(lat) = START_LAT + (lat - 1) * 1.0
      end do
      do lon = 1, NLONS
         lons(lon) = START_LON+(1.0/2) + (lon - 1) * 1.0
      end do
      ...
      do rec1 = 1,NRECS
         ...
         recs(rec1) = rec1 -1  + jd
         jd = jd + jdmon(imonth) - 1
         imonth=imonth + 1
         if (mod(rec1,12) == 0)then
           iyear = iyear + 1
           imonth=1
         endif
c         write(6,*) recs(rec1)
