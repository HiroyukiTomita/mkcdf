...
C     pressure fields.
      ...
      parameter (VAR_NAME='QA',
     +           VAR_NAME_LONG='J-OFURO3 Specific Humidity')
      integer var_varid
...
c     Open input data file
      fni="/home/tomita/MULTI_QA/DATA/V0.3"
     &  //"/J-OFURO3_QA_MULTI_V0.3_2008.bin"
...
C     similarly for (lon_dimid).
     ...
     +     lon_varid)
      if (retval .ne. nf_noerr) call handle_err(retval)
      retval = nf_def_var(ncid, REC_NAME, NF_INT, 1, rec_dimid, 
     +     rec_varid)
      if (retval .ne. nf_noerr) call handle_err(retval)
...
C     and longitudes of our data grid into the netCDF file.
      ...
      if (retval .ne. nf_noerr) call handle_err(retval)
      retval = nf_put_var_int(ncid, rec_varid, recs)
      if (retval .ne. nf_noerr) call handle_err(retval)
      write(6,*) "OK 5"
...
C     a real application, the data would change between timesteps.
      do rec1 = 1, NRECS
         ...
         do j=1,NLATS
          read(50)(var_out(i,j,1),i=1,NLONS)
         enddo
