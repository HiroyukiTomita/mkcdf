c mk_ofuro_nc_mwir_v1.1.f 
c
c This is a sample program to create netCDF file 
c for J-OFURO3 Daily mwir grid variable for a year
c
c PARAMETERS
...
C     timesteps of data.
      integer NDIMS, NRECS
      parameter (NDIMS = 3, NRECS = JJDD)
      integer NLATS, NLONS
      parameter (NLATS = 2048, NLONS = 4096)
...
C     have some real data to write, for example, model output.
      do lat = 1, NLATS
         lats(lat) = START_LAT+(0.087890625/2) + (lat - 1) * 0.087890625
      end do
      do lon = 1, NLONS
         lons(lon) = START_LON+(0.087890625/2) + (lon - 1) * 0.087890625
      end do
      do rec1 = 1,NRECS
         recs(rec1) = rec1 -1
